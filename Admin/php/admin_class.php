<?php
   class admin_db{
    ...
    public function select_users(){
        $sql = "SELECT imie,nazwisko,ID FROM users";
        $result = $this->conn->query($sql);
        if(@$result->num_rows > 0){
            while($row = $result->fetch_array(MYSQLI_ASSOC)){
                echo "<tr>" . "<td>" . $row["imie"] . "</td>" . "<td>" . $row["nazwisko"] . "</td>" . "<td> <button class='btn btn-info btn-user-show' id=" . $row["ID"] .">Pokaż</button><button class='btn btn-danger btn-user-delete' id=" . $row["ID"] ." >Delete</button>" . "</tr>";
            }
        }
    } 
